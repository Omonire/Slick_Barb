...
__name(newReference, "newReference");
...
__name(json, "json");
var hitStore = /* @__PURE__ */ new Map();
var WINDOW_MS = 6e4;
var MAX_PER_WINDOW = 6;
...
function notifyText(record) {
  const lines = [
    `New booking ${record.reference}`,
    `${record.client_name} \u2014 ${record.service_name ?? record.service_id}`,
    record.barber_name ? `with ${record.barber_name}` : "first available barber",
    `${record.booking_date} at ${record.booking_time ? to12h(record.booking_time) : ""}`,
    `Contact: ${record.contact}`
  ];
  if (record.notes) lines.push(`Notes: ${record.notes}`);
  return lines.join("\n");
}
__name(notifyText, "notifyText");
async function storeBooking(env, record) {
  if (!env.DATABASE_URL) return false;
  let sql;
  try {
    sql = src_default(env.DATABASE_URL, { max: 1, connect_timeout: 8 });
    ...
    if (sql) {
      await sql.end().catch(() => void 0);
    }
    return false;
  }
}
__name(storeBooking, "storeBooking");
async function onRequestPost(ctx) {
  ...
  const notes = typeof body.notes === "string" ? body.notes.trim().slice(0, 1e3) : "";
  ...
  if (ctx.env.NOTIFY_WEBHOOK_URL) {
    try {
      await fetch(ctx.env.NOTIFY_WEBHOOK_URL, {
        method: "POST",
        headers: {
          "Content-Type": "text/plain; charset=utf-8",
          Title: `Slicks \u2014 ${record.client_name}`
        },
        body: notifyText(record)
      });
    } catch (err) {
...
// ../.wrangler/tmp/pages-S4SXMk/functionsRoutes-0.38049643748496786.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-VYKbkB/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-VYKbkB/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.06184551880796385.mjs.map
